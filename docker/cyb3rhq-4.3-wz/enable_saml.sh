...
indexer="$1-cyb3rhq.indexer-1"
dashboard="$1-cyb3rhq.dashboard-1"

# Setup keycloack to be used with cyb3rhq-dashboards
...
PC="{
  ...
  \"name\": \"cyb3rhq\",
  \"clientId\": \"cyb3rhq\",
  \"description\": \"cyb3rhq saml integration\",
  ...
  \"attributes\" : {
    ...
    \"saml_single_logout_service_url_post\": \"https://cyb3rhq.dashboard:5601/_opendistro/_security/saml/logout\",
...
curl -sS -L -X POST "${B}/admin/realms/${REALM}/clients" "${H[@]}" -d "$PC" | grep -v "Client cyb3rhq already exists"

# Get a client json representation
CLIENT=$(curl -sS -L -X GET "${B}/admin/realms/${REALM}/clients" "${H[@]}" -G -d 'clientId=cyb3rhq' |jq '.[] | select(.clientId=="cyb3rhq")')
...
PU='{
  ...
  "emailVerified": true,
  "enabled": true,
  "credentials": [{"temporary":false,"type":"password","value":"cyb3rhq"}],
...
USER=$(curl -sS -L -X GET "${B}/admin/realms/${REALM}/users" "${H[@]}" -G -d 'username=cyb3rhq' |jq '.[] | select(.username=="cyb3rhq")')
...
certs="/usr/share/cyb3rhq-indexer/config/certs"
ca="$certs/ca.pem"
cert="$certs/admin.pem"
key="$certs/admin-key.pem"

securityadmin="bash /usr/share/cyb3rhq-indexer/plugins/opensearch-security/tools/securityadmin.sh"
config_path="/usr/share/cyb3rhq-indexer/plugins/opensearch-security/securityconfig/"

echo "To update configuration in indexer, you can run:"
echo docker exec -e JAVA_HOME=/usr/share/cyb3rhq-indexer/jdk $indexer $securityadmin -cacert $ca -cert $cert -key $key -cd $config_path
