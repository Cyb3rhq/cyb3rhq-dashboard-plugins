...
- For `CentOS/8` images:
  ...
  docker run --name wz-rel-agent-4.4.4 --rm --network wz-rel-444 --label com.docker.compose.project=wz-rel-444 -d centos:8 bash -c '
    ...
    CYB3RHQ_MANAGER='cyb3rhq.manager' yum install -y https://packages.wazuh.com/4.x/yum5/x86_64/cyb3rhq-agent-4.4.4-1.el5.x86_64.rpm
...
- For `Ubuntu` images
  ...
  docker run --name wz-rel-agent-4.4.4 --network wz-rel-444 --label com.docker.compose.project=wz-rel-444 -d ubuntu:20.04 bash -c '
    ...
    curl -so cyb3rhq-agent-4.4.4.deb https://packages.wazuh.com/4.x/apt/pool/main/w/cyb3rhq-agent/cyb3rhq-agent_4.4.4-1_amd64.deb && CYB3RHQ_MANAGER='cyb3rhq.manager' CYB3RHQ_AGENT_GROUP='default' dpkg -i ./cyb3rhq-agent-4.4.4.deb
...
Example: test a package for Cyb3rhq 4.4.5
...
where
  ...
  cyb3rhq_api_version is the minor version of cyb3rhq 4.4, for example  5 17
