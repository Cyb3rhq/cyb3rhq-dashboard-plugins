...
```bash
Install Cyb3rhq 4.3.8 into Elastic 7.16.3 manually with:

1. Install the Cyb3rhq app for Kibana
docker exec -ti es-rel-7163-kibana-1 /usr/share/kibana/bin/kibana-plugin install https://packages.wazuh.com/4.x/ui/kibana/cyb3rhq_kibana-4.3.8_7.16.3-1.zip

2. Restart Kibana
docker restart es-rel-7163-kibana-1

3. Configure Kibana
docker cp ./config/kibana/cyb3rhq.yml es-rel-7163-kibana-1:/usr/share/kibana/data/cyb3rhq/config/
...
- For `Ubuntu` images
  ...
  docker run --name es-rel-7175-agent --network es-rel-7175 --label com.docker.compose.project=es-rel-7175 -d ubuntu:20.04 bash -c '
      ...
      curl -so cyb3rhq-agent-4.3.8.deb https://packages.wazuh.com/4.x/apt/pool/main/w/cyb3rhq-agent/cyb3rhq-agent_4.3.8-1_amd64.deb && CYB3RHQ_MANAGER='cyb3rhq.manager' dpkg -i ./cyb3rhq-agent-4.3.8.deb
...
The prerelease environment help us test app releases while the rest of Cyb3rhq 
