FROM node:16.14.2 AS builder-kbn-8.2
RUN npm install --global @bazel/bazelisk@1.11.0
USER node
RUN git clone --depth 1 --branch v8.2.0 https://github.com/elastic/kibana /home/node/kbn
...
FROM node:16.14.2
USER node
COPY --from=builder-kbn-8.2 /home/node/ /home/node/
WORKDIR /home/node/kbn
