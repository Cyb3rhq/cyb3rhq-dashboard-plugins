FROM node:16.13.0 AS builder-kbn-7.16.1
RUN npm install --global @bazel/bazelisk@1.10.1
USER node
RUN git clone --depth 1 --branch v7.16.1 https://github.com/elastic/kibana /home/node/kbn
...
FROM node:16.13.0
USER node
COPY --from=builder-kbn-7.16.1 /home/node/ /home/node/
WORKDIR /home/node/kbn
