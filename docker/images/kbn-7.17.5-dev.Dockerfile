FROM node:16.14.2 AS builder-kbn-7.17.5
RUN npm install --global @bazel/bazelisk@1.10.1
USER node
RUN git clone --depth 1 --branch v7.17.5 https://github.com/elastic/kibana /home/node/kbn
...
FROM node:16.14.2
USER node
COPY --from=builder-kbn-7.17.5 /home/node/ /home/node/
WORKDIR /home/node/kbn
