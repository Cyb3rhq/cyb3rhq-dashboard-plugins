...
export COMPOSE_PROJECT_NAME=wz-rel-${CYB3RHQ_STACK//./}

profile="standard"
export CYB3RHQ_DASHBOARD_CONF=./config/cyb3rhq_dashboard/cyb3rhq_dashboard.yml
export SEC_CONFIG_FILE=./config/cyb3rhq_indexer/config.yml

if [[ "$3" =~ "saml" ]]; then
	profile="saml"
	export CYB3RHQ_DASHBOARD_CONF=./config/cyb3rhq_dashboard/cyb3rhq_dashboard_saml.yml
	export SEC_CONFIG_FILE=./config/cyb3rhq_indexer/config-saml.yml
fi
...
up)
	...
	echo "  curl -so \cyb3rhq-agent-${CYB3RHQ_STACK}.deb \\"
	echo "    https://packages.wazuh.com/4.x/apt/pool/main/w/cyb3rhq-agent/cyb3rhq-agent_${CYB3RHQ_STACK}-1_amd64.deb \\"
	echo "    && CYB3RHQ_MANAGER='cyb3rhq.manager' CYB3RHQ_AGENT_GROUP='default' dpkg -i ./cyb3rhq-agent-${CYB3RHQ_STACK}.deb"
	echo
