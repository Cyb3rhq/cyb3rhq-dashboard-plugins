...
import {
  ...
  CYB3RHQ_CORE_CONFIGURATION_CACHE_SECONDS,
  CYB3RHQ_DATA_CONFIG_APP_PATH,
} from '../common/constants';
import { enhanceConfiguration } from './services/enhance-configuration';

export class Cyb3rhqCorePlugin
  implements Plugin<Cyb3rhqCorePluginSetup, Cyb3rhqCorePluginStart>
{
  ...
  public async setup(
    core: CoreSetup,
    plugins: PluginSetup,
  ): Promise<Cyb3rhqCorePluginSetup> {
    this.logger.debug('cyb3rhq_core: Setup');
    ...
    this._internal.configurationStore = new ConfigurationStore(
      ...
      {
        cache_seconds: CYB3RHQ_CORE_CONFIGURATION_CACHE_SECONDS,
        file: CYB3RHQ_DATA_CONFIG_APP_PATH,
      },
    );
    ...
    core.http.registerRouteHandlerContext('cyb3rhq_core', (context, request) => {
      return {
        ...this.services,
        api: {
          client: {
  ...
  public async start(core: CoreStart): Promise<Cyb3rhqCorePluginStart> {
    this.logger.debug('cyb3rhqCore: Started');
