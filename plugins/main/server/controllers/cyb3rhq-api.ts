/*
 * Cyb3rhq app - Class for Cyb3rhq-API functions
...
export class Cyb3rhqApiCtrl {
  ...
  async getToken(
    ...
    try {
      ...
      const { username } = await context.cyb3rhq.security.getCurrentUser(
        request,
        context,
      );
      if (
      ...
      let token;
      if (context.cyb3rhq_core.manageHosts.isEnabledAuthWithRunAs(idHost)) {
        token = await context.cyb3rhq.api.client.asCurrentUser.authenticate(
          idHost,
        );
      } else {
        token = await context.cyb3rhq.api.client.asInternalUser.authenticate(
          idHost,
        );
      }

      let textSecure = '';
      if (context.cyb3rhq.server.info.protocol === 'https') {
        textSecure = ';Secure';
      }
  ...
  /**
   * Returns if the cyb3rhq-api configuration is working
  ...
  async checkStoredAPI(
    ...
    try {
      ...
      context.cyb3rhq.logger.debug(`Getting server API host by ID: ${id}`);
      const apiHostData = await context.cyb3rhq_core.manageHosts.get(id, {
      ...
      context.cyb3rhq.logger.debug(`${id} exists`);
      ...
      // If we have a valid response from the Cyb3rhq API
      if (
        ...
        if (responseAgents.status === HTTP_STATUS_CODES.OK) {
          ...
          if (api.cluster_info) {
            // Update cluster information in the cyb3rhq-registry.json
            await context.cyb3rhq_core.manageHosts.updateRegistryByHost(
              id,
              api.cluster_info,
            );
      ...
      // If we have an invalid response from the Cyb3rhq API
      throw new Error(
      ...
      if (error.code === 'EPROTO') {
        ...
        try {
          const apis = await context.cyb3rhq_core.manageHosts.get();
  ...
  /**
   * This check the cyb3rhq-api configuration received in the POST body will work
   * @param {Object} context
   * @param {Object} request
   * @param {Object} response
   * @returns {Object} status obj or ErrorResponse
  ...
  async checkAPI(
    ...
    try {
      ...
      context.cyb3rhq.logger.debug(`${request.body.id} is valid`);
      // Check if a Cyb3rhq API id is given (already stored API)
      const data = await context.cyb3rhq_core.manageHosts.get(request.body.id, {
        excludePassword: true,
      });
      if (data) {
        apiAvailable = data;
        ...
        context.cyb3rhq.logger.debug(errorMessage);
        return ErrorResponse(
          errorMessage,
          3029,
      ...
      context.cyb3rhq.logger.debug(`${request.body.id} credentials are valid`);
      if (
        responseManagerInfo.status === HTTP_STATUS_CODES.OK &&
        responseManagerInfo.data
      ) {
        const result =
          await context.cyb3rhq_core.manageHosts.getRegistryDataByHost(data);
        return response.ok({
          body: result,
        });
      }
  ...
  /**
   * Check main Cyb3rhq daemons status
   * @param {*} context Endpoint context
   * @param {*} api API entry stored in .cyb3rhq
   * @param {*} path Optional. Cyb3rhq API target path.
   */
  async checkDaemons(context, api, path) {
    try {
      const response = await context.cyb3rhq.api.client.asInternalUser.request(
        'GET',
        '/manager/status',
        {},
        { apiHostID: api.id },
      ...
      const isCluster =
        ...
        typeof daemons['cyb3rhq-clusterd'] !== 'undefined';
      ...
      const isValid = execd && modulesd && cyb3rhqdb && clusterd;

      isValid && context.cyb3rhq.logger.debug('Cyb3rhq is ready');
  ...
  /**
   * This performs a request over Cyb3rhq API and returns its response
  ...
  async makeRequest(context, method, path, data, id, response) {
    ...
    try {
      let api;
      try {
        api = await context.cyb3rhq_core.manageHosts.get(id, {
          excludePassword: true,
        });
      } catch (error) {
        context.cyb3rhq.logger.error('Could not get host credentials');
        //Can not get credentials from cyb3rhq-hosts
        return ErrorResponse(
      ...
      if (delay) {
        ...
        addJobToQueue({
          ...
          run: async contextJob => {
            try {
              await context.cyb3rhq.api.client.asCurrentUser.request(
                method,
                path,
                data,
                options,
              ...
              contextJob.cyb3rhq.logger.error(
      ...
      context.cyb3rhq.logger.debug(`${method} ${path}`);
      ...
      context.cyb3rhq.logger.error(errorMsg || error);
      if (devTools) {
        return response.ok({
  ...
  requestApi(
    ...
    if (!request.body.method) {
      ...
      context.cyb3rhq.logger.error('Request method is not valid.');
      ...
      context.cyb3rhq.logger.error('Request path is not valid.');
  ...
  /**
   * Get full data on CSV format from a list Cyb3rhq API endpoint
   * @param {Object} ctx
   * @param {Object} request
   * @param {Object} response
  ...
  async csv(
    ...
    try {
      ...
      context.cyb3rhq.logger.debug(`Report ${tmpPath}`);
      ...
      const output = await context.cyb3rhq.api.client.asCurrentUser.request(
        'GET',
        `/${tmpPath}`,
        { params: params },
      ...
      if (totalItems && !isList) {
        ...
        while (itemsArray.length < totalItems && params.offset < totalItems) {
          ...
          const tmpData = await context.cyb3rhq.api.client.asCurrentUser.request(
            'GET',
            `/${tmpPath}`,
            { params: params },
            { apiHostID: request.body.id },
      ...
      if (totalItems) {
        ...
        throw new Error(
          `An error occurred fetching data from the Cyb3rhq API${
  ...
  /**
   * This get the cyb3rhq setup settings
   * @param {Object} context
   * @param {Object} request
   * @param {Object} response
  ...
  async getSetupInfo(
    ...
    try {
      return response.ok({
        body: {
          ...
          data: {
            ...
            configuration_file: context.cyb3rhq_core.configuration.store.file,
          },
        },
      });
    } catch (error) {
      context.cyb3rhq.logger.error(error.message || error);
      return ErrorResponse(
        `Could not get data from cyb3rhq-version registry due to ${
          error.message || error
        }`,
        4005,
        HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR,
  ...
  async getSyscollector(
    ...
    try {
      ...
      const data = await Promise.all([
        context.cyb3rhq.api.client.asInternalUser.request(
          'GET',
  ...
  async getAppLogos(
    ...
    try {
      ...
      const logos = {
        [APP_LOGO]:
          await context.cyb3rhq_core.configuration.getCustomizationSetting(
            APP_LOGO,
          ),
        [HEALTHCHECK_LOGO]:
          await context.cyb3rhq_core.configuration.getCustomizationSetting(
