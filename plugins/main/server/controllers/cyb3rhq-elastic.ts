/*
 * Cyb3rhq app - Class for Cyb3rhq-Elastic functions
...
import {
  CYB3RHQ_SAMPLE_ALERTS_INDEX_SHARDS,
  CYB3RHQ_SAMPLE_ALERTS_INDEX_REPLICAS,
...
import {
  CYB3RHQ_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS,
  CYB3RHQ_SAMPLE_ALERTS_DEFAULT_NUMBER_ALERTS,
} from '../../common/constants';
import { CYB3RHQ_INDEXER_NAME } from '../../common/constants';
import { routeDecoratorProtectedAdministrator } from './decorators';

export class Cyb3rhqElasticCtrl {
  constructor() {}
  ...
  async buildSampleIndexByCategory(
    ...
    return `${await context.cyb3rhq_core.configuration.get(
  ...
  async getTemplate(
    ...
    try {
      ...
      if (!templates || typeof templates !== 'string') {
        throw new Error(
          `An unknown error occurred when fetching templates from ${CYB3RHQ_INDEXER_NAME}`,
        );
      }
      ...
      return ErrorResponse(
        `Could not retrieve templates from ${CYB3RHQ_INDEXER_NAME} due to ${
          error.message || error
        }`,
        4002,
        500,
  ...
  async getCurrentPlatform(
    ...
    try {
      return response.ok({
        body: {
          platform: context.cyb3rhq.security.platform,
        },
      });
    } catch (error) {
      context.cyb3rhq.logger.error(error.message || error);
   ...
   * @param {String} id Index-pattern id to use in the visualizations. Eg: 'cyb3rhq-alerts'
   */
  async buildVisualizationsRaw(context, app_objects, id, namespace = false) {
    ...
    let monitoringPattern = `${config['cyb3rhq.monitoring.pattern']}`;
    context.cyb3rhq.logger.debug(`Building ${app_objects.length} visualizations`);
    context.cyb3rhq.logger.debug(`Index pattern ID: ${id}`);
    const visArray = [];
    ...
    for (let element of app_objects) {
      ...
      if (
        ...
        const isMonitoring = defaultStr.includes('cyb3rhq-monitoring');
        if (isMonitoring) {
          ...
          aux_source.kibanaSavedObjectMeta.searchSourceJSON =
            defaultStr.replace(/cyb3rhq-alerts/g, id);
        }
      }

      // Replace index-pattern for selector visualizations
      if (typeof (aux_source || {}).visState === 'string') {
        aux_source.visState = aux_source.visState.replace(/cyb3rhq-alerts/g, id);
      }
   ...
   * @param {String} name Cluster name. Eg: 'cyb3rhq'
  ...
  buildClusterVisualizationsRaw(
    ...
    try {
      ...
      for (const element of app_objects) {
        ...
        aux_source = aux_source.replace(/cyb3rhq-alerts/g, id);
  ...
  async haveSampleAlerts(
    ...
    try {
      // Check if cyb3rhq sample alerts index exists
      const results = await Promise.all(
        Object.keys(CYB3RHQ_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS).map(
          async category =>
  ...
  /**
   * This creates sample alerts in cyb3rhq-sample-alerts
  ...
  createSampleAlerts = routeDecoratorProtectedAdministrator(1000)(
    async (
      ...
      try {
        ...
        const sampleAlerts = CYB3RHQ_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS[
          request.params.category
        ]
          .map(typeAlert =>
            generateAlerts(
        ...
        if (!existsSampleIndex.body) {
          // Create cyb3rhq sample alerts index

          const configuration = {
            settings: {
              index: {
                number_of_shards: CYB3RHQ_SAMPLE_ALERTS_INDEX_SHARDS,
                number_of_replicas: CYB3RHQ_SAMPLE_ALERTS_INDEX_REPLICAS,
              },
            },
          };
          ...
          context.cyb3rhq.logger.info(`Index ${sampleAlertsIndex} created`);
        }
  ...
  deleteSampleAlerts = routeDecoratorProtectedAdministrator(1000)(
    async (
      ...
      // Delete Cyb3rhq sample alert index
      const sampleAlertsIndex = await this.buildSampleIndexByCategory(
        context,
        request.params.category,
      );
      ...
      try {
        ...
        if (existsSampleIndex.body) {
          // Delete Cyb3rhq sample alerts index
          await context.core.opensearch.client.asCurrentUser.indices.delete({
            index: sampleAlertsIndex,
          });
          context.cyb3rhq.logger.info(`Deleted ${sampleAlertsIndex} index`);
          return response.ok({
