/*
 * Cyb3rhq app - Class for Cyb3rhq reporting controller
...
export class Cyb3rhqReportingCtrl {
  constructor() {}
  /**
    ...
    try {
      ...
      if (section && typeof section === 'string') {
        if (!['agentConfig', 'groupConfig'].includes(section)) {
          printer.addContent({
            text: CYB3RHQ_MODULES[tab].title + ' report',
            style: 'h1',
          });
      ...
      if (CYB3RHQ_MODULES[tab] && CYB3RHQ_MODULES[tab].description) {
        printer.addContentWithNewLine({
          text: CYB3RHQ_MODULES[tab].description,
          style: 'standard',
        });
      }
    } catch (error) {
  ...
  private getConfigRows(context, data, labels) {
    context.cyb3rhq.logger.debug('Building configuration rows');
    const result = [];
  ...
  private getConfigTables(context, data, section, tab, array = []) {
    context.cyb3rhq.logger.debug('Building configuration tables');
    let plainData = {};
    const nestedData = [];
    const tableData = [];
  ...
  createReportsModules = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.cyb3rhq.logger.debug('Report started');
        const {
          array,
          agents,
          browserTimezone,
        ...
        const printer = new ReportPrinter(
          context.cyb3rhq.logger.get('report-printer'),
          context.cyb3rhq_core.configuration,
        );
        ...
        createDirectoryIfNotExists(
          path.join(
            CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
            context.cyb3rhqEndpointParams.hashUsername,
          ),
        );
        ...
        if (time) {
          additionalTables = await extendedInformation(
            ...
            agentsFilter,
            indexPatternTitle ||
              context.cyb3rhq_core.configuration.getSettingValue('pattern'),
            agents,
          );
        }
        ...
        await printer.print(context.cyb3rhqEndpointParams.pathFilename);

        return response.ok({
          body: {
            success: true,
            message: `Report ${context.cyb3rhqEndpointParams.filename} was created`,
            filename: context.cyb3rhqEndpointParams.filename,
          },
        });
      } catch (error) {
  ...
  createReportsGroups = this.checkReportsUserDirectoryIsValidRouteDecorator(
    ...
    ({ params: { groupID } }) =>
      `cyb3rhq-group-configuration-${groupID}-${this.generateReportTimestamp()}.pdf`,
  );
  ...
  createReportsAgentsConfiguration =
    this.checkReportsUserDirectoryIsValidRouteDecorator(
      async (
        ...
        try {
          ...
          try {
            ...
            context.cyb3rhq.logger.debug(error.message || error);
          }
      ...
      ({ params: { agentID } }) =>
        `cyb3rhq-agent-configuration-${agentID}-${this.generateReportTimestamp()}.pdf`,
    );
    ...
    this.checkReportsUserDirectoryIsValidRouteDecorator(
      async (
        ...
        try {
          ...
          const { hashUsername } = await context.cyb3rhq.security.getCurrentUser(
            request,
            context,
          );
          createDataDirectoryIfNotExists();
          ...
          context.cyb3rhq.logger.debug('Syscollector report');
          const [sanitizedFilters, agentsFilter] = filters
            ? this.sanitizeKibanaFilters(context, filters, searchBar)
            : [false, null];
          ...
          const requestInventory = async agentRequestInventory => {
            try {
              context.cyb3rhq.logger.debug(agentRequestInventory.loggerMessage);
      ...
      ({ params: { agentID } }) =>
        `cyb3rhq-agent-inventory-${agentID}-${this.generateReportTimestamp()}.pdf`,
    );
  ...
  async getReports(
    ...
    try {
      context.cyb3rhq.logger.debug('Fetching created reports');
      const { hashUsername } = await context.cyb3rhq.security.getCurrentUser(
        request,
        context,
      );
      ...
      context.cyb3rhq.logger.debug(`Directory: ${userReportsDirectoryPath}`);
      ...
      context.cyb3rhq.logger.debug(`Total reports: ${reports.length}`);
      return response.ok({
        body: { reports },
      });
    } catch (error) {
  ...
  getReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.cyb3rhq.logger.debug(
          `Getting ${context.cyb3rhqEndpointParams.pathFilename} report`,
        );
        const reportFileBuffer = fs.readFileSync(
          context.cyb3rhqEndpointParams.pathFilename,
        );
        return response.ok({
  ...
  deleteReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.cyb3rhq.logger.debug(
          `Deleting ${context.cyb3rhqEndpointParams.pathFilename} report`,
        );
        fs.unlinkSync(context.cyb3rhqEndpointParams.pathFilename);
        context.cyb3rhq.logger.info(
          `${context.cyb3rhqEndpointParams.pathFilename} report was deleted`,
        );
        return response.ok({
          body: { error: 0 },
        });
    ...
    return async (
      ...
      try {
        const { username, hashUsername } =
          await context.cyb3rhq.security.getCurrentUser(request, context);
        const userReportsDirectoryPath = path.join(
          CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
          hashUsername,
        );
        ...
        return await routeHandler.bind(this)(
          {
            ...context,
            cyb3rhqEndpointParams: { hashUsername, filename, pathFilename },
          },
          request,
          response,
        );
