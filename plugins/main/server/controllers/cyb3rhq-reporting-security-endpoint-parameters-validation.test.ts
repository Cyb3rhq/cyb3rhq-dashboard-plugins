...
import { Cyb3rhqReportingRoutes } from '../routes/cyb3rhq-reporting';
import md5 from 'md5';
import {
...
import {
  ...
  CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
...
beforeAll(async () => {
  // Create <PLUGIN_PLATFORM_PATH>/data/cyb3rhq directory.
  createDataDirectoryIfNotExists();
  // Create <PLUGIN_PLATFORM_PATH>/data/cyb3rhq/downloads directory.
  createDirectoryIfNotExists(CYB3RHQ_DATA_DOWNLOADS_DIRECTORY_PATH);
  // Create <PLUGIN_PLATFORM_PATH>/data/cyb3rhq/downloads/reports directory.
  createDirectoryIfNotExists(CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
  // Create report files
  [
    { name: md5('admin'), files: ['cyb3rhq-module-overview-general-1234.pdf'] },
    {
      name: md5('../../etc'),
      files: ['cyb3rhq-module-overview-general-1234.pdf'],
    },
  ].forEach(({ name, files }) => {
    createDirectoryIfNotExists(
      path.join(CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, name),
    );
...
afterAll(async () => {
  // Remove <PLUGIN_PLATFORM_PATH>/data/cyb3rhq directory.
  execSync(`rm -rf ${CYB3RHQ_DATA_ABSOLUTE_PATH}`);
...
describe('[endpoint][security] GET /reports/{name} - Parameters validation', () => {
  it.each`
    ...
    ${'Get report by filename'} | ${'admin'}     | ${'cyb3rhq-module-overview-general-1234.pdf'}               | ${200}             | ${null}
    ${'Invalid parameters'}     | ${'admin'}     | ${'..%2fcyb3rhq-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'}     | ${'admin'}     | ${'custom..%2fcyb3rhq-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}        | ${'admin'}     | ${'../custom..%2fcyb3rhq-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
    ${'Get report by filename'} | ${'../../etc'} | ${'cyb3rhq-module-overview-general-1234.pdf'}               | ${200}             | ${null}
    ${'Invalid parameters'}     | ${'../../etc'} | ${'..%2fcyb3rhq-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'}     | ${'../../etc'} | ${'custom..%2fcyb3rhq-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}        | ${'../../etc'} | ${'../custom..%2fcyb3rhq-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
  `(
    ...
    async ({
      ...
      const response = await supertest(innerServer.listener)
        ...
        .send({
          ...
          tables: [],
          section: 'overview',
          indexPatternTitle: 'cyb3rhq-alerts-*',
          apiId: 'default',
        })
        .expect(responseStatusCode);
      if (responseBodyMessage) {
...
describe('[endpoint][security] DELETE /reports/{name} - Parameters validation', () => {
  it.each`
    testTitle               | username       | filename                                               | responseStatusCode | responseBodyMessage
    ${'Delete report file'} | ${'admin'}     | ${'cyb3rhq-module-overview-general-1234.pdf'}            | ${200}             | ${null}
    ${'Invalid parameters'} | ${'admin'}     | ${'..%2fcyb3rhq-module-overview-general-1234.pdf'}       | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'} | ${'admin'}     | ${'custom..%2fcyb3rhq-module-overview-general-1234.pdf'} | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}    | ${'admin'}     | ${'../cyb3rhq-module-overview-general-1234.pdf'}         | ${404}             | ${/Not Found/}
    ${'Delete report file'} | ${'../../etc'} | ${'cyb3rhq-module-overview-general-1234.pdf'}            | ${200}             | ${null}
    ${'Invalid parameters'} | ${'../../etc'} | ${'..%2fcyb3rhq-module-overview-general-1234.pdf'}       | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'} | ${'../../etc'} | ${'custom..%2fcyb3rhq-module-overview-general-1234.pdf'} | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}    | ${'../../etc'} | ${'../cyb3rhq-module-overview-general-1234.pdf'}         | ${404}             | ${/Not Found/}
  `(
