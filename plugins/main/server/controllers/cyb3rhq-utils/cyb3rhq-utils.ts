...
export class Cyb3rhqUtilsCtrl {
  ...
  async getConfiguration(
    ...
    try {
      context.cyb3rhq.logger.debug('Getting configuration');
  ...
  updateConfiguration = compose(
    ...
    async (
      ...
      const { requirements, update: updatedConfiguration } =
        await context.cyb3rhq_core.configuration.set(updatedSettings);
      context.cyb3rhq.logger.debug('Configuration updated');
  ...
  uploadFile = compose(
    ...
    async (
      ...
      const pluginSetting = context.cyb3rhq_core.configuration._settings.get(key);
      ...
      context.cyb3rhq.logger.debug(`Directory: ${targetDirectory}`);
      ...
      context.cyb3rhq.logger.debug(`Storing file on : ${files.join(', ')}`);
      ...
      const { requirements, update } =
        await context.cyb3rhq_core.configuration.set(updatedConfiguration);
  ...
  deleteFile = compose(
    ...
    async (
      ...
      // Update the setting in the configuration cache
      const { requirements, update } =
        await context.cyb3rhq_core.configuration.clear(key);
