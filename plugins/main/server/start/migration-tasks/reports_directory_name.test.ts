...
describe('[migration] Rename the subdirectories of `reports` directory', () => {
  ...
  afterEach(() => {
    mockContext = null;
    execSync(`rm -rf ${CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH}`);
  });
  ...
  const userDirectoriesTest5 = [
    { ...userNameDirectory1, errorRenaming: true },
    userNameDirectory1MD5WithFiles,
    userNameDirectory2,
  ];
  const userDirectoriesTest6 = [
    { ...userNameDirectory1, errorRenaming: true },
    userNameDirectory1MD5WithFiles,
    { ...userNameDirectory2, errorRenaming: true },
    userNameDirectory2MD5WithFiles,
  ];
  const userDirectoriesTest7 = [
    userNameDirectory1WithFiles,
    userNameDirectory2WithFiles,
  ];
  const userDirectoriesTest8 = [
    userNameDirectory1MD5WithFiles,
    userNameDirectory2MD5WithFiles,
  ];
          ...
          .map(
            input =>
              `[${input.name}:${input.files}${
                input.errorRenaming ? ' (Error: renaming)' : ''
              }]`,
          )
          .join(', ')
      : 'None';
  ...
  `(
    'Migrate Directories: $title - FoundRequireRenamingDirectories: $foundRequireRenamingDirectories - renamedDirectories: $renamedDirectories.',
    ({ directories, foundRequireRenamingDirectories, renamedDirectories }) => {
      const errorRenamingDirectoryMessages =
      ...
      expect(
        mockContext
          ._getLogs()
          .filter(({ message }) =>
            message.includes('Found reports directory to migrate'),
          ),
      ).toHaveLength(foundRequireRenamingDirectories);
      // Check the quantity of directories were renamed.
      expect(
        mockContext
          ._getLogs()
          .filter(({ message }) => message.includes('Renamed directory [')),
      ).toHaveLength(renamedDirectories);
      expect(
        mockContext
          ._getLogs('error')
          .filter(({ message }) =>
            message.includes(`Error renaming directory [`),
          ),
      ).toHaveLength(errorRenamingDirectoryMessages);
        ...
        if (!rest.errorRenaming) {
          if (isMD5(name)) {
            ...
            expect(
              mockContext
                ._getLogs()
                .filter(({ message }) =>
                  message.includes(`Renamed directory [${name}`),
                ),
            ).toHaveLength(1);
            expect(
              mockContext
                ...
                .filter(({ message }) =>
                  message.includes(
                    `Found reports directory to migrate: [${name}`,
                  ),
                ),
            ).toHaveLength(1);
            expect(
          ...
          expect(
            mockContext
              ._getLogs('error')
              .some(({ message }) =>
                message.includes(`Error renaming directory [${name}`),
              ),
          ).toBe(true);
          expect(
            fs.existsSync(
          ...
          expect(
            fs.existsSync(
              path.join(CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, md5(name)),
            ),
          ).toBe(true);
        }
      });
