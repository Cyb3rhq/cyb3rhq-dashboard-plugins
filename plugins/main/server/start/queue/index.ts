/*
 * Cyb3rhq app - Add delayed jobs to a queue.
...
import { CYB3RHQ_QUEUE_CRON_FREQ } from '../../../common/constants';
...
async function executePendingJobs(context: any) {
  try {
    ...
    context.cyb3rhq.logger.debug(`Pending jobs: ${pendingJobs.length}`);
...
export function jobQueueRun(context) {
  cron.schedule(CYB3RHQ_QUEUE_CRON_FREQ, async () => {
    try {
