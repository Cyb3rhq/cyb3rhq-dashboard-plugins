// To launch this file
// yarn test:jest --testEnvironment node --verbose server/routes/cyb3rhq-reporting
...
import { Cyb3rhqUtilsCtrl } from '../controllers/cyb3rhq-utils/cyb3rhq-utils';
import md5 from 'md5';
...
beforeAll(async () => {
  ...
  // Create <PLUGIN_PLATFORM_PATH>/data/cyb3rhq/config directory.
  createDirectoryIfNotExists(CYB3RHQ_DATA_CONFIG_DIRECTORY_PATH);
...
describe('[endpoint] GET /reports', () => {
  ...
  beforeAll(() => {
    ...
    directories.forEach(({ username, files }) => {
      ...
      createDirectoryIfNotExists(
        path.join(CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, hashUsername),
      );
      if (files) {
  ...
  afterAll(async () => {
    // Remove <PLUGIN_PLATFORM_PATH>/data/cyb3rhq/downloads directory.
    execSync(`rm -rf ${CYB3RHQ_DATA_DOWNLOADS_DIRECTORY_PATH}`);
  });
...
describe('[endpoint] PUT /utils/configuration', () => {
  ...
  beforeAll(() => {
    ...
    Object.entries(SettingsDefinitions).forEach(([key, value]) =>
      context.cyb3rhq_core.configuration._settings.set(key, value),
    );
  });
  ...
  `(
    ...
    async ({ footer, header, responseStatusCode, expectedMD5, tab }) => {
      ...
      context.cyb3rhq_core.configuration.getCustomizationSetting.mockImplementation(
        (...settings) => ({
          then: fn =>
            fn(
              Object.fromEntries(
      ...
      context.cyb3rhq_core.dashboardSecurity.isAdministratorUser.mockImplementation(
        () => ({ administrator: true }),
      );

      // Set custom report header and footer
      if (typeof footer === 'string' || typeof header === 'string') {
        context.cyb3rhq_core.configuration.set.mockReturnValueOnce({
      ...
      const reportPath = `${CYB3RHQ_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH}/${userPath}/${fileName}`;
