...
import { Cyb3rhqApiCtrl } from '../controllers';
import { schema } from '@osd/config-schema';

export function Cyb3rhqApiRoutes(router: IRouter) {
  const ctrl = new Cyb3rhqApiCtrl();
  ...
  // Check if credentials on POST connect to Cyb3rhq API. Not storing them!
  // Returns if the cyb3rhq-api configuration received in the POST body will work
  router.post(
    {
      path: '/api/check-api',
      validate: {
  ...
  // Returns data from the Cyb3rhq API on CSV readable format
  router.post(
    {
      path: '/api/csv',
      validate: {
