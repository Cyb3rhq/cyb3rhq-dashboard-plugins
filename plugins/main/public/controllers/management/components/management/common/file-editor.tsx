...
class WzFileEditor extends Component {
   ...
   */
  async save(name, overwrite = true, relativeDirname = '') {
    if (!this._isMounted) {
      return;
    } else if (/\s/.test(name)) {
      this.showToast(
    ...
    try {
      ...
      if (error instanceof Error) {
        errorMessage = error.details ? error.details : String(error);
      }
...
export default connect(mapStateToProps, mapDispatchToProps)(WzFileEditor);
