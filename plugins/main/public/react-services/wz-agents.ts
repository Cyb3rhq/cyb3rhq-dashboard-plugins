/*
 * Cyb3rhq app - Services related to agents
...
import { CYB3RHQ_AGENTS_OS_TYPE } from '../../common/constants';
...
export function getAgentOSType(agent) {
  if (agent?.os?.uname?.toLowerCase().includes(CYB3RHQ_AGENTS_OS_TYPE.LINUX)) {
    return CYB3RHQ_AGENTS_OS_TYPE.LINUX;
  } else if (agent?.os?.platform === CYB3RHQ_AGENTS_OS_TYPE.WINDOWS) {
    return CYB3RHQ_AGENTS_OS_TYPE.WINDOWS;
  } else if (agent?.os?.platform === CYB3RHQ_AGENTS_OS_TYPE.SUNOS) {
    return CYB3RHQ_AGENTS_OS_TYPE.SUNOS;
  } else if (agent?.os?.platform === CYB3RHQ_AGENTS_OS_TYPE.DARWIN) {
    return CYB3RHQ_AGENTS_OS_TYPE.DARWIN;
  } else {
    return CYB3RHQ_AGENTS_OS_TYPE.OTHERS;
  }
}
