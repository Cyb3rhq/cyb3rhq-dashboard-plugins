...
export const getUpdates = async (
  ...
  try {
    ...
    const { manageHosts, api: cyb3rhqApiClient } = getCyb3rhqCore();
    ...
    const apisAvailableUpdates = await Promise.all(
      hosts?.map(async api => {
        ...
        try {
          const response = await cyb3rhqApiClient.client.asInternalUser.request(
            method,
            path,
            data,
            options,
    ...
    const { logger } = getCyb3rhqCheckUpdatesServices();
